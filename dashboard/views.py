...
from .forms import ProductForm, CategoryForm, UpdateOrderForm
...
# @login_required()
# @permission_required("perms.is_staff",login_url='login')



# Dashboard Order views

def dashboard_orders(request):
    orders = Order.objects.all().order_by('-created_at')
    print(orders)
    context = {
        "jsFunction":'startTime()',
        "logo": logo,
        "orders":orders,
    }
    return render(request, 'dashboard/dashboard_orders.html', context)

def dashboard_order_details(request, uid):
    main_order = Order.objects.get(uid=uid)
    orders = OrderItem.objects.filter(order=main_order)
    form = UpdateOrderForm(request.POST or None)
    form.fields["status"].initial = main_order.status
    # form.fields["is_paid"] = main_order.is_paid
    form.is_paid = main_order.is_paid
    form.payment_method = main_order.payment_method
    print(form.is_paid)
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            paid = request.POST.get('is_paid', False)
            if paid=="on":
                is_paid = True
            else:
                is_paid = False
            payment_method = form.cleaned_data.get("payment_method")
            main_order.status = status
            main_order.is_paid = is_paid
            main_order.payment_method = payment_method
            main_order.save()
            messages.success(request, 'Order updated successfully!')
            return redirect("dashboard-order-details", uid=uid)
    context = {
        "jsFunction":'startTime()',
        "logo": logo,
        "orders":orders,
    ...
    return render(request, 'dashboard/dashboard_order_details.html', context)
...
def add_category(request):
    ...
    return render(request, 'dashboard/dashboard_add_edit_category.html', context)
...
def category_details(request, slug):
    ...
    return render(request,'dashboard/dashboard_category_details.html', context)
...
def product_details(request,slug):
    ...
    return render(request,"dashboard/dashboard_product_details.html",context)
...
def add_product(request):
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            product.slug = slugify(title, allow_unicode=True)
            form.save()
    ...
    return render(request,"dashboard/dashboard_add_edit_product.html",context)
...
def delete_product(request, pk):
    ...
    #     return reverse(request, "dashboard-product-details" +"/"+)
